                        ...
                        @foreach($products as $product)
                            ...
                            <div class="col-lg-3">
                                <div class="card" style="margin-bottom: 20px; height: auto;">
                                    <img src="/images/{{ $product->PRO_IMG }}"
                                        class="card-img-top mx-auto"
                                        style="height: 150px; width: 150px;display: block;"
                                        alt="{{ $product->PRO_IMG }}"
                                    >
                                    <div class="card-body">
                                        <a href=""><h6 class="card-title">{{ $product->PRO_NAME }}</h6></a>
                                        <p>precio: ${{ $product->PRO_PRICE }}</p>
                                        <p>unidades disponibles: {{ $product->unidades }}</p>
                                        <form action="{{ route('cart.store') }}" method="POST">
                                            {{ csrf_field() }}
                                            <input type="hidden" value="{{ $product->ID }}" id="id" name="id">
                                            <input type="hidden" value="{{ $product->PRO_NAME }}" id="name" name="name">
                                            <input type="hidden" value="{{ $product->PRO_PRICE }}" id="price" name="price">
                                            <input type="hidden" value="{{ $product->PRO_IMG }}" id="img" name="img">
                                            <input type="hidden" value="1" id="quantity" name="quantity">

                                            <div class="card-footer" style="background-color: white;">
                                                <div class="row">
                                                    <button class="btn btn-secondary btn-sm" type= "submit" class="tooltip-test" title="add to cart" id = "boton">
